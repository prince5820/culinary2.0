              ...
              {links.map((link) =>
                link.groups && link.groups.length > 0 ? (
                  <NavigationMenuPrimitive.Item key={link.href}>
                    <NavigationMenuPrimitive.Trigger className="group/button flex w-full items-center justify-between p-3 ps-0 font-semibold hover:text-[#d1093f] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20">
                      <CustomLink
                        className="font-semibold hover:text-[#d1093f]"
                        href={link.href}
                        ...
                        <ul className="flex flex-col" key={group.href}>
                          <li>
                            <NavigationMenuPrimitive.Link asChild>
                              <CustomLink
                                className="block w-full p-3 ps-0 font-semibold hover:text-[#d1093f]"
                                href={group.href}
                                onClick={() => setOpen(false)}
                              >
                                {group.label}
                          ...
                          {group.links &&
                            ...
                            group.links.map((nestedLink) => (
                              <li key={nestedLink.href}>
                                <NavigationMenuPrimitive.Link asChild>
                                  <CustomLink
                                    className="block w-full p-3 ps-0 hover:text-[#d1093f]"
